      ...
      <body style={{ margin: 0, minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#FFFFFF', color: '#0E0F0C', fontFamily: 'Inter, system-ui, sans-serif', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 420 }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button onClick={() => reset()} style={{ background: '#9FE870', color: '#163300', border: 'none', borderRadius: 9999, padding: '10px 20px', fontWeight: 600, cursor: 'pointer' }}>
              Réessayer
            </button>
            <button onClick={() => window.location.reload()} style={{ background: 'transparent', color: '#163300', borderWidth: 1, borderStyle: 'solid', borderColor: '#163300', borderRadius: 9999, padding: '10px 20px', fontWeight: 600, cursor: 'pointer' }}>
              Recharger
            </button>
          </div>
        </div>
